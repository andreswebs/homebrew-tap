...
REPO_NAME="feedwatch"
FORMULA_FILE="Formula/feedwatch.rb"
...
main() {
    echo_stderr "Starting Feedwatch formula update check..."
    ...
    darwin_amd64_sha=$(get_sha256_from_api "${latest_version}" "feedwatch-darwin-amd64-v${latest_version}.tar.gz")
    darwin_arm64_sha=$(get_sha256_from_api "${latest_version}" "feedwatch-darwin-arm64-v${latest_version}.tar.gz")
    linux_amd64_sha=$(get_sha256_from_api "${latest_version}" "feedwatch-linux-amd64-v${latest_version}.tar.gz")
    linux_arm64_sha=$(get_sha256_from_api "${latest_version}" "feedwatch-linux-arm64-v${latest_version}.tar.gz")
    ...
    if git diff --quiet "${FORMULA_FILE}"; then
        ...
        echo_stderr "To commit: git add ${FORMULA_FILE} && git commit -m \"feedwatch: update to ${latest_version}\""
    fi
