...
REPO_NAME="codelens"
FORMULA_FILE="Formula/codelens.rb"
...
main() {
    echo_stderr "Starting Codelens formula update check..."
    ...
    darwin_amd64_sha=$(get_sha256_from_api "${latest_version}" "codelens-darwin-amd64-v${latest_version}.tar.gz")
    darwin_arm64_sha=$(get_sha256_from_api "${latest_version}" "codelens-darwin-arm64-v${latest_version}.tar.gz")
    linux_amd64_sha=$(get_sha256_from_api "${latest_version}" "codelens-linux-amd64-v${latest_version}.tar.gz")
    linux_arm64_sha=$(get_sha256_from_api "${latest_version}" "codelens-linux-arm64-v${latest_version}.tar.gz")
    ...
    if git diff --quiet "${FORMULA_FILE}"; then
        ...
        echo_stderr "To commit: git add ${FORMULA_FILE} && git commit -m \"codelens: update to ${latest_version}\""
    fi
