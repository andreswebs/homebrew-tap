...
REPO_NAME="terminology"
FORMULA_FILE="Formula/terminology.rb"
...
main() {
    echo_stderr "Starting Terminology formula update check..."
    ...
    darwin_amd64_sha=$(get_sha256_from_api "${latest_version}" "terminology-darwin-amd64-v${latest_version}.tar.gz")
    darwin_arm64_sha=$(get_sha256_from_api "${latest_version}" "terminology-darwin-arm64-v${latest_version}.tar.gz")
    linux_amd64_sha=$(get_sha256_from_api "${latest_version}" "terminology-linux-amd64-v${latest_version}.tar.gz")
    linux_arm64_sha=$(get_sha256_from_api "${latest_version}" "terminology-linux-arm64-v${latest_version}.tar.gz")
    ...
    if git diff --quiet "${FORMULA_FILE}"; then
        ...
        echo_stderr "To commit: git add ${FORMULA_FILE} && git commit -m \"terminology: update to ${latest_version}\""
    fi
